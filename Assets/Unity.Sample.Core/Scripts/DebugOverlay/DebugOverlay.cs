...
public class DebugOverlay : MonoBehaviour
{
    ...
    public static void DrawLineAbsolute(float x1, float y1, float x2, float y2, Color col)
    {
        if (instance == null)
            return;
        float scalex = (float)instance.width / Screen.width;
    ...
    public static void DrawAngle(float posX, float posY, float size, float angle, Color color)
    {
        var dir3D = Quaternion.AngleAxis(angle * -1 + 180, Vector3.up) * Vector3.forward;
        DrawVector(posX, posY, dir3D.x, dir3D.z, size, color);
    }

    public static void DrawVector(float posX, float posY, float dirX, float dirY, float size, Color color)
    {
        posX = posX * Screen.width;
        posY = posY * Screen.height;
        size = size * (Screen.width + Screen.height) * 0.5f;

        var p0 = new Vector2(posX + 0f * size, posY + 1f * size);
        var p1 = new Vector2(posX + .7f * size, posY + .7f * size);
        var p2 = new Vector2(posX + 1f * size, posY + 0f * size);
        var p3 = new Vector2(posX + .7f * size, posY + -.7f * size);
        var p4 = new Vector2(posX + 0f * size, posY + -1f * size);
        var p5 = new Vector2(posX + -.7f * size, posY + -.7f * size);
        var p6 = new Vector2(posX + -1f * size, posY + 0f * size);
        var p7 = new Vector2(posX + -.7f * size, posY + .7f * size);

        DrawLineAbsolute(p0.x, p0.y, p1.x, p1.y, color);
        DrawLineAbsolute(p1.x, p1.y, p2.x, p2.y, color);
        DrawLineAbsolute(p2.x, p2.y, p3.x, p3.y, color);
        DrawLineAbsolute(p3.x, p3.y, p4.x, p4.y, color);
        DrawLineAbsolute(p4.x, p4.y, p5.x, p5.y, color);
        DrawLineAbsolute(p5.x, p5.y, p6.x, p6.y, color);
        DrawLineAbsolute(p6.x, p6.y, p7.x, p7.y, color);
        DrawLineAbsolute(p7.x, p7.y, p0.x, p0.y, color);
        DrawLineAbsolute(posX, posY, posX + dirX * size * 0.9f, posY + dirY * size * 0.9f, color);
    }
