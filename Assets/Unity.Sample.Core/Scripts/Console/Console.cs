...
{
    void Init(string buildId, string buildUnityVersion);
    void Shutdown();
...
//        AddCommand("waitload", CmdWaitLoad, "Wait for level load");
    ...
    public static void ProcessCommandLineArguments(string[] arguments)
    {
        ...
        Debug.Log("ProcessCommandLineArguments: " + string.Join(" ", arguments));
    ...
    {
        var lastMsgTime = s_FrameTime - timeLastMsg;
        if (lastMsgTime < 1.0)
            Overlay.Managed.Write(0, 0, lastMsg);
...
// TODO (mogensh) Can we somehow define these outside console as they depend on level load     
...
//    }
    ...
    public static void EnqueueCommandNoHistory(string command)
    {
        ...
        if (s_PendingCommands.Contains(command))
        {
            GameDebug.Log("Duplicate command found! Not Enqueuing " + command);
            return;
        }
