...
#pragma warning disable 649
    ...
    [ConfigVar(Name = "r.aamode", DefaultValue = "fxaa", Description = "AA mode: off, fxaa, smaa, taa", Flags = ConfigVar.Flags.Save)]
...
#pragma warning restore 649
    ...
    public static void Init()
    {
        ...
        // Default to reasonable window
        if (rResolution.Value == "")
            rResolution.Value = "1280x720@" + Screen.currentResolution.refreshRate;

        currentQualityIdx = default;
        currentResX = default;
        currentResY = default;
        currentResRate = default;
    }
    ...
    public static void Update()
    {
        ...
        {
            if (currentResX != Screen.width || currentResY != Screen.height || currentResRate != Screen.currentResolution.refreshRate)
            {
        ...
        //    FilmGrain.globalEnable = rGrain.IntValue > 0;

        if (rSSR.ChangeCheck())
        {
            // Disabled when we removed the changes to HDRP
            //ScreenSpaceReflection.globalEnable = rSSR.IntValue > 0;
            updateFrameSettings = true;
        }
        ...
        //    hdasset.m_RenderPipelineSettings.decalSettings.drawDistance = rDecalDist.IntValue;
        //}
        ...
        if (updateAAFlags)
            UpdateAAFlags(GameApp.CameraStack.TopCamera());

        if (updateFrameSettings)
            UpdateFrameSettings(GameApp.CameraStack.TopCamera());
    }
    ...
    {
        Overlay.Managed.Write(2, 2, "Quality Settings");
        Overlay.Managed.Write(2, 4, "Current camera: {0}", GameApp.CameraStack.TopCamera() != null ? GameApp.CameraStack.TopCamera().name : "(null)");
        //Overlay.Managed.Write(2, 5, "Post volume:    {0}", "");// Game.game.TopCamera() != null ? Game.game.TopCamera().name : "(null)");

        Overlay.Managed.Write(2, 7, "r.aamode:          {0}", rAAMode.Value);
        Overlay.Managed.Write(2, 8, "r.aaquality:       {0}", rAAQuality.Value);
        Overlay.Managed.Write(2, 9, "r.bloom:           {0}", rBloom.IntValue);
        Overlay.Managed.Write(2, 10, "r.motionblur:      {0}", rMotionBlur.IntValue);
        Overlay.Managed.Write(2, 11, "r.ssao:            {0}", rSSAO.IntValue);
        Overlay.Managed.Write(2, 12, "r.grain:           {0}", rGrain.IntValue);
        Overlay.Managed.Write(2, 13, "r.ssr:             {0}", rSSR.IntValue);

        Overlay.Managed.Write(2, 15, "r.sss:             {0}", rSSS.IntValue);
        Overlay.Managed.Write(2, 16, "r.roughrefraction: {0}", rRoughRefraction.IntValue);
        Overlay.Managed.Write(2, 17, "r.distortion:      {0}", rDistortion.IntValue);

        Overlay.Managed.Write(2, 19, "r.decaldist:       {0}", rDecalDist.IntValue);
        Overlay.Managed.Write(2, 20, "r.shadowdistmult:  {0}", rShadowDistMult.FloatValue);
    }
