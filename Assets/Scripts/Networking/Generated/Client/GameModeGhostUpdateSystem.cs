...
public class GameModeGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
...
#pragma warning restore 649
#endif
        [ReadOnly] public ArchetypeChunkBufferType<GameModeSnapshotData> ghostSnapshotDataType;
        [ReadOnly] public ArchetypeChunkEntityType ghostEntityType;
        public ArchetypeChunkComponentType<GameModeData> ghostGameModeDataType;
        ...
        public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
        {
            ...
            var ghostGameModeDataArray = chunk.GetNativeArray(ghostGameModeDataType);
#if UNITY_EDITOR || DEVELOPMENT_BUILD
...
#endif
                GameModeSnapshotData snapshotData;
                snapshot.GetDataAtTick(targetTick, targetTickFraction, out snapshotData);

                var ghostGameModeData = ghostGameModeDataArray[entityIndex];
                ghostGameModeData.gameTimerSeconds = snapshotData.GetGameModeDatagameTimerSeconds(deserializerState);
                ghostGameModeData.gameTimerMessage = snapshotData.GetGameModeDatagameTimerMessage(deserializerState);
                ghostGameModeData.teamName0 = snapshotData.GetGameModeDatateamName0(deserializerState);
                ghostGameModeData.teamName1 = snapshotData.GetGameModeDatateamName1(deserializerState);
                ghostGameModeData.teamScore0 = snapshotData.GetGameModeDatateamScore0(deserializerState);
                ghostGameModeData.teamScore1 = snapshotData.GetGameModeDatateamScore1(deserializerState);
                ghostGameModeDataArray[entityIndex] = ghostGameModeData;
            }
        }
    }
    [BurstCompile]
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<GameModeSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadWrite<GameModeData>(),
            },
        ...
        {
            All = new []{
                ComponentType.ReadOnly<GameModeSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadOnly<PredictedGhostComponent>(),
                ComponentType.ReadWrite<GameModeData>(),
            }
        });
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<GameModeSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        if (!m_predictedQuery.IsEmptyIgnoreFilter)
        {
            var updatePredictedJob = new UpdatePredictedJob
            {
                ...
                ghostSnapshotDataType = GetArchetypeChunkBufferType<GameModeSnapshotData>(true),
                ...
                ghostGameModeDataType = GetArchetypeChunkComponentType<GameModeData>(),
...
public partial class GameModeGhostSpawnSystem : DefaultGhostSpawnSystem<GameModeSnapshotData>
{
    ...
    protected override JobHandle SetPredictedGhostDefaults(NativeArray<GameModeSnapshotData> snapshots, NativeArray<int> predictionMask, JobHandle inputDeps)
    {
